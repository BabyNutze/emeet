		...
		<div class="row">
			<div class="col-md-12">
				<div class="page-header clearfix">
					...
					<div class="float-right"><a href="?menu=agenda&sub=addagenda" class="btn btn-info">เพิ่มการประชุม</a>
				</div>
			...
			$sql = "SELECT *, TIME_FORMAT(start_time, '%H:%i') as st,TIME_FORMAT(end_time, '%H:%i') as et,
			DATE_FORMAT(meeting_day, '%d/%m/%Y') as md
			...
			if ( $result = mysqli_query( $conn, $sql ) ) {
				if ( mysqli_num_rows( $result ) > 0 ) {
					...
					while ( $row = mysqli_fetch_array( $result ) ) {
						...
						echo "<td><a href='home.php?menu=agenda&sub=viewagenda&a=$agenda_id' >" . $row[ 'agenda_subject' ] . " ครั้งที่ " . $row["round"] . "</a></td>";
						echo "<td>";
